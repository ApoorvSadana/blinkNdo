...
while True:
        ...
        for rect in rects:
                ...
                equ_left_resized = cv2.resize(equ_left, (24,8), interpolation = cv2.INTER_AREA) 
                gray_right = cv2.cvtColor(right_eye_image, cv2.COLOR_BGR2GRAY)
                equ_right = cv2.equalizeHist(gray_right)
                equ_right_resized = cv2.resize(equ_right, (24,8), interpolation = cv2.INTER_AREA) 
...
##              ret,thresh_right = cv2.threshold(gray_right,THRESH_CONST,255,cv2.THRESH_BINARY_INV)
                ...
                if minLoc_left[0]<=6 and minLoc_right[0]<=8:
                        # print("right")
                        left = 0
                        right = 1
                elif minLoc_left[0]>=14 and minLoc_right[0]>=17:
                        # print("left")
                        left = 1
                        right = 0
                else:
                ...
                if directionCounter_left>10:
                        execute_left = 1
                        execute_right = 0
                        # print("Execute Left")
                elif directionCounter_right>10:
                        execute_right = 1
                        execute_left = 0
                        # print("Execite Right")
                
                if blinked==1 and execute_left==1:
                        os.system("python /home/apoorv/Desktop/blinkNdo/phone_call_updated.py")
                        print("Made Phone Call!")
                        execute_left = 0
                        execute_right = 0
                        directionCounter_left = 0
                        directionCounter_right = 0
                elif blinked==1 and execute_right==1:
                ...
                cv2.imshow("Minimum Intensity Left Eye", equ_left_copy)
                cv2.imshow("Minimum Intensity Right Eye", equ_right_copy)
